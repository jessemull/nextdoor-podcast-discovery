...
class TestSessionManager:
    ...
    @pytest.fixture
    def session_manager(self, encryption_key: bytes, mock_supabase: mock.MagicMock) -> SessionManager:
        """Create a SessionManager instance with mocked dependencies."""
        with mock.patch("src.session_manager.create_client", return_value=mock_supabase):
            with mock.patch.dict(os.environ, {"SESSION_ENCRYPTION_KEY": encryption_key.decode(), "SUPABASE_URL": "https://test.supabase.co", "SUPABASE_SERVICE_KEY": "test_key"}):
                return SessionManager()

    def test_get_cookies_returns_none_when_no_session(self, session_manager: SessionManager) -> None:
        """Should return None when no session exists."""
        result_mock = mock.MagicMock()
        result_mock.data = []
        session_manager.supabase.table.return_value.select.return_value.order.return_value.limit.return_value.execute.return_value = result_mock

        result = session_manager.get_cookies()

        assert result is None

    def test_get_cookies_returns_cookies_when_valid(self, session_manager: SessionManager, encryption_key: bytes) -> None:
    ...
    def test_get_cookies_returns_none_when_expired(self, session_manager: SessionManager, encryption_key: bytes) -> None:
    ...
    def test_get_cookies_handles_invalid_token(self, session_manager: SessionManager) -> None:
        """Should return None when encryption key is invalid."""
        # Use wrong key to encrypt
        wrong_key = Fernet.generate_key()
        encrypted = Fernet(wrong_key).encrypt(json.dumps([{"name": "test", "value": "cookie"}]).encode())
        expires_at = (datetime.now(UTC) + timedelta(days=1)).isoformat()
    ...
    def test_save_cookies_encrypts_and_saves(self, session_manager: SessionManager) -> None:
        """Should encrypt and save cookies to database."""
        cookies = [{"name": "test", "value": "cookie"}]
        result_mock = mock.MagicMock()
        session_manager.supabase.table.return_value.upsert.return_value.execute.return_value = result_mock
    ...
    def test_save_cookies_uses_default_session_id(self, session_manager: SessionManager) -> None:
    ...
    def test_save_cookies_uses_provided_neighborhood_id(self, session_manager: SessionManager) -> None:
