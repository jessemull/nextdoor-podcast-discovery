...
help:
	...
	@echo ""
	@echo "Quality:"
	@echo "  lint             Run linters (scraper + web). Scraper: ruff format --check, ruff check, mypy (no writes). Web: eslint. Run \`make format\` first if Python formatting fails."
	@echo "  lint-ci          Same as lint but web uses eslint --fix (CI runs \`make format\` before this)"
	@echo "  lint-scraper     Python: ruff format --check, ruff check, mypy (does not modify files)"
